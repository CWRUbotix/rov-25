...
bool RH_MRF89::init() {
  ...
  if (spiReadRegister(RH_MRF89_REG_02_FDEVREG) != 0xaa) return false;
  spiWriteRegister(RH_MRF89_REG_02_FDEVREG, 0x3);  // Back to the default for FDEV
  if (spiReadRegister(RH_MRF89_REG_02_FDEVREG) != 0x3) return false;
  ...
  spiWriteRegister(
    RH_MRF89_REG_00_GCONREG, RH_MRF89_CMOD_STANDBY | RH_MRF89_FBS_902_915 | RH_MRF89_VCOT_60MV);
  spiWriteRegister(
  ...
  spiWriteRegister(
    RH_MRF89_REG_1A_TXCONREG, 0xf0 | RH_MRF89_TXOPVAL_13DBM);  // TX cutoff freq=375kHz,
  spiWriteRegister(RH_MRF89_REG_1B_CLKOREG, 0x00);             // Disable clock output to save power
  spiWriteRegister(
    RH_MRF89_REG_1C_PLOADREG, 0x40);  // payload=64bytes (no RX-filtering on packet length)
  spiWriteRegister(RH_MRF89_REG_1D_NADDSREG, 0x00);  // Node Address (0=default) Not used
  spiWriteRegister(
    RH_MRF89_REG_1E_PKTCREG, RH_MRF89_PKTLENF | RH_MRF89_PRESIZE_4 | RH_MRF89_WHITEON |
  ...
  if (!setFrequency(915.4) && !setFrequency(865.0)) return false;
  // Some slow, reliable default speed and modulation
  if (!setModemConfig(FSK_Rb20Fd40)) return false;
...
bool RH_MRF89::send(const uint8_t* data, uint8_t len) {
  if (len > RH_MRF89_MAX_MESSAGE_LEN) return false;
...
uint8_t RH_MRF89::maxMessageLength() { return RH_MRF89_MAX_MESSAGE_LEN; }
...
bool RH_MRF89::verifyPLLLock() {
  ...
  while ((millis() - ulStartTime < 1000)) {
    ...
    if ((ftpriVal & RH_MRF89_LSTSPLL) != 0) break;
  }
  setOpMode(RH_MRF89_CMOD_STANDBY);
