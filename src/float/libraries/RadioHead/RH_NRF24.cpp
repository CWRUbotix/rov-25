...
uint8_t RH_NRF24::flushTx() { return spiCommand(RH_NRF24_COMMAND_FLUSH_TX); }

uint8_t RH_NRF24::flushRx() { return spiCommand(RH_NRF24_COMMAND_FLUSH_RX); }
...
bool RH_NRF24::send(const uint8_t* data, uint8_t len) {
  if (len > RH_NRF24_MAX_MESSAGE_LEN) return false;
...
bool RH_NRF24::printRegisters() {
  ...
  for (uint8_t r = RH_NRF24_REG_00_CONFIG; r <= RH_NRF24_REG_1D_FEATURE; r++) {
    if ((r <= RH_NRF24_REG_17_FIFO_STATUS) || (r >= RH_NRF24_REG_1C_DYNPD)) {
      ...
      if (
        (RH_NRF24_REG_0A_RX_ADDR_P0 == r) || (RH_NRF24_REG_0B_RX_ADDR_P1 == r) ||
...
uint8_t RH_NRF24::maxMessageLength() { return RH_NRF24_MAX_MESSAGE_LEN; }
