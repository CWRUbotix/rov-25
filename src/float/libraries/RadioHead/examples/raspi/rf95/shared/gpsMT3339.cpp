...
char* gps_MT3339::getLatitude() { return latitude; }

char* gps_MT3339::getLongitude() { return longitude; }

char* gps_MT3339::getTimestamp() { return timestamp; }
...
void* gps_MT3339::read_gps(void* ptr) {
  ...
  while (!stop_reading) {
    if (gps_fd < 0) {
      ...
      if (n <= 0) {
        if (
          ((errno != EAGAIN) and (errno != EWOULDBLOCK)) || (empty_read_counter > MAX_EMPTY_READ)) {
        ...
        if (start_ptr != NULL) {
          end_ptr = (char*)memchr(start_ptr, '*', LEN_GPS_READ_BUFFER - (start_ptr - gps_buffer));

          if (end_ptr != NULL) {
            gga_len = (end_ptr - start_ptr) < LEN_GPS_SENTENCE_BUFFER ? (end_ptr - start_ptr)
                                                                      : LEN_GPS_SENTENCE_BUFFER - 1;
            ...
            while (end_ptr != NULL && start_ptr != NULL) {
              ...
              if (end_ptr != NULL) {
                ...
                int temp = nofchar;
                if (comma_count == 0) {
                  nofchar = temp < LEN_GPS_INFO_BUFFER ? temp : LEN_GPS_INFO_BUFFER - 1;
                  strncpy(timestamp, start_ptr, nofchar);
                  timestamp[nofchar] = '\0';
                }
                if (comma_count == 1) {
                  nofchar = temp < LEN_GPS_INFO_BUFFER - 2 ? temp : LEN_GPS_INFO_BUFFER - 2;
...
void gps_MT3339::stop() { stop_reading = true; }
