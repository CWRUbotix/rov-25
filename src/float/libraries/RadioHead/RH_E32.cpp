...
: _s(s), _m0_pin(m0_pin), _m1_pin(m1_pin), _aux_pin(aux_pin) {
...
bool RH_E32::init() {
  ...
  if (!getVersion()) return false;  // Could not communicate with module or wrong type of module

  setMode(RHModeRx);
  clearRxBuf();

  if (!setDataRate(DataRate5kbps)) return false;

  if (!setPower(Power21dBm)) return false;

  //  if (!setBaudRate(BaudRate9600, Parity8N1))
  //  return false;

  if (!setFrequency(433)) return false;
...
bool RH_E32::readParameters(Parameters& params) {
  setOperatingMode(ModeSleep);
  uint8_t readParamsCommand[] = {
    RH_E32_COMMAND_READ_PARAMS, RH_E32_COMMAND_READ_PARAMS, RH_E32_COMMAND_READ_PARAMS};
...
bool RH_E32::writeParameters(Parameters& params, bool save) {
  ...
  if (result != sizeof(params)) return false;
...
bool RH_E32::getVersion() {
  setOperatingMode(ModeSleep);
  uint8_t readVersionCommand[] = {
    RH_E32_COMMAND_READ_VERSION, RH_E32_COMMAND_READ_VERSION, RH_E32_COMMAND_READ_VERSION};
...
void RH_E32::waitAuxLow() { while (digitalRead(_aux_pin) == true); }

// Check whether the latest received message is complete and uncorrupted
void RH_E32::validateRxBuf() {
  if (_bufLen < RH_E32_HEADER_LEN) return;  // Too short to be a real message
  if (_bufLen != _buf[0]) return;           // Do we have all the message?
...
bool RH_E32::recv(uint8_t* buf, uint8_t* len) {
  ...
  if (buf && len) {
    ...
    if (*len > _bufLen - RH_E32_HEADER_LEN) *len = _bufLen - RH_E32_HEADER_LEN;
...
bool RH_E32::send(const uint8_t* data, uint8_t len) {
  if (len > RH_E32_MAX_MESSAGE_LEN) return false;
...
uint8_t RH_E32::maxMessageLength() { return RH_E32_MAX_MESSAGE_LEN; }

bool RH_E32::waitPacketSent() {
  if (_mode == RHModeTx) waitAuxHigh();
  setMode(RHModeRx);
  return true;
}
...
bool RH_E32::setDataRate(DataRate rate) {
  Parameters params;
  if (!readParameters(params)) return false;
...
bool RH_E32::setFrequency(uint16_t frequency) {
  if (frequency < 410 || frequency > 441) return false;
