...
bool RH_NRF51::setChannel(uint8_t channel) {
  NRF_RADIO->FREQUENCY =
    ((channel << RADIO_FREQUENCY_FREQUENCY_Pos) & RADIO_FREQUENCY_FREQUENCY_Msk);
  return true;
}
...
bool RH_NRF51::send(const uint8_t* data, uint8_t len) {
  if (len > RH_NRF51_MAX_MESSAGE_LEN) return false;

  #if RH_NRF51_HAVE_ENCRYPTION
  if (_encrypting && len > RH_NRF51_MAX_ENCRYPTED_MESSAGE_LEN) return false;
  #endif
...
bool RH_NRF51::available() {
  if (!_rxBufValid) {
    if (_mode == RHModeTx) return false;
    setModeRx();
    if (!NRF_RADIO->EVENTS_END) return false;  // No message yet
    setModeIdle();
  #if RH_NRF51_HAVE_ENCRYPTION
    ...
    validateRxBuf();
    if (!_rxBufValid) setModeRx();  // Try for another
  }
  return _rxBufValid;
}
...
uint8_t RH_NRF51::maxMessageLength() {
  #if RH_NRF51_HAVE_ENCRYPTION
  if (_encrypting) return RH_NRF51_MAX_ENCRYPTED_MESSAGE_LEN;
  #endif
