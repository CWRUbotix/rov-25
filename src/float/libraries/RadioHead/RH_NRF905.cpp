...
RH_NRF905::RH_NRF905(
  uint8_t chipEnablePin, uint8_t txEnablePin, uint8_t slaveSelectPin, RHGenericSPI& spi)
...
bool RH_NRF905::setChannel(uint16_t channel, bool hiFrequency) {
  ...
  if (hiFrequency) reg1 |= RH_NRF905_CONFIG_1_HFREQ_PLL;
...
bool RH_NRF905::send(const uint8_t* data, uint8_t len) {
  if (len > RH_NRF905_MAX_MESSAGE_LEN) return false;
...
bool RH_NRF905::waitPacketSent() {
  ...
  while (!(statusRead() & RH_NRF905_STATUS_DR)) YIELD;
  setModeIdle();
  return true;
}
...
void RH_NRF905::validateRxBuf() {
  // Check the length
  uint8_t len = _buf[4];
  if (len > RH_NRF905_MAX_MESSAGE_LEN) return;  // Silly LEN header
...
uint8_t RH_NRF905::maxMessageLength() { return RH_NRF905_MAX_MESSAGE_LEN; }
