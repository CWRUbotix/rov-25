...
////////////////////////////////////////////////////////////////////
// Public methods
void RHReliableDatagram::setTimeout(uint16_t timeout) { _timeout = timeout; }

////////////////////////////////////////////////////////////////////
void RHReliableDatagram::setRetries(uint8_t retries) { _retries = retries; }
    ...
    while ((timeLeft = timeout - (millis() - thisSendTime)) > 0) {
      if (waitAvailableTimeout(timeLeft)) {
        ...
        if (recvfrom(0, 0, &from, &to, &id, &flags))  // Discards the message
        {
          ...
          if (
            from == address && to == _thisAddress && (flags & RH_FLAGS_ACK) &&
...
////////////////////////////////////////////////////////////////////
bool RHReliableDatagram::recvfromAck(
  uint8_t* buf, uint8_t* len, uint8_t* from, uint8_t* to, uint8_t* id, uint8_t* flags) {
  uint8_t _from;
  uint8_t _to;
  uint8_t _id;
  uint8_t _flags;
...
// the ack with a define in say platformio.ini:
        ...
        while ((millis() - ts) <= RH_ACK_DELAY) YIELD;
#endif
      ...
      if (
        (RH_ENABLE_EXPLICIT_RETRY_DEDUP && !(_flags & RH_FLAGS_RETRY)) || _id != _seenIds[_from]) {
        if (from) *from = _from;
        if (to) *to = _to;
        if (id) *id = _id;
...
bool RHReliableDatagram::recvfromAckTimeout(
  ...
  while ((timeLeft = timeout - (millis() - starttime)) > 0) {
    if (waitAvailableTimeout(timeLeft)) {
      if (recvfromAck(buf, len, from, to, id, flags)) return true;
    }
    YIELD;
  }
  return false;
