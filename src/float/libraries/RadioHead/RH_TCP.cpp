  ...
  if (count < 0) {
    ...
    while (socketBufLen >= 5) {
      ...
      if (len > sizeof(socketBuf) - sizeof(message->length)) {
        ...
        fprintf(
          stderr,
          "RH_TCP::checkForEvents read ridiculous length: %d. Corrupt message stream? Aborting\n",
          len);
        close(_socket);
        _socket = -1;
        return false;
...
bool RH_TCP::available() {
  if (_socket < 0) return false;
  if (checkForEvents()) return false;  // Som sort of IO failre
  if (_rxBufFull) {
    validateRxBuf();
    _rxBufFull = false;
  }
...
bool RH_TCP::recv(uint8_t* buf, uint8_t* len) {
  ...
  if (buf && len) {
    if (*len > _rxBufLen) *len = _rxBufLen;
...
bool RH_TCP::send(const uint8_t* data, uint8_t len) {
  if (!waitCAD()) return false;  // Check channel activity (prob not possible for this driver?)
...
uint8_t RH_TCP::maxMessageLength() { return RH_TCP_MAX_MESSAGE_LEN; }
