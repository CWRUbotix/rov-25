...
bool RH_LoRaFileOps::recv(uint8_t* buf, uint8_t* len) {
  ...
  if (sz < 4) return false;  // Too short to be a real message
  ...
  if (_promiscuous || _rxHeaderTo == _thisAddress || _rxHeaderTo == RH_BROADCAST_ADDRESS) {
    ...
    if (*len > (sz - RH_LORAFILEOPS_HEADER_LEN)) *len = sz - RH_LORAFILEOPS_HEADER_LEN;
...
bool RH_LoRaFileOps::send(const uint8_t* data, uint8_t len) {
  ...
  if (len > RH_LORAFILEOPS_MAX_MESSAGE_LEN) return false;
...
uint8_t RH_LoRaFileOps::maxMessageLength() { return RH_LORAFILEOPS_MAX_MESSAGE_LEN; }
...
void RH_LoRaFileOps::setSpreadingFactor(int32_t sf) { ioctl(_fd, LORA_SET_SPRFACTOR, &sf); }
...
int RH_LoRaFileOps::lastSNR() { return _lastSNR; }
