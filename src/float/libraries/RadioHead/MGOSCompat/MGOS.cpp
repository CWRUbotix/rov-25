 ...
 */
uint32_t millis(void) { return (uint32_t)mgos_uptime_micros() / 1000; }

/**
 * @brief Provide a delay in milliseconds.
 * @param ms The number of Milli Seconds to delay.
 */
void delay(unsigned long ms) { mgos_msleep(ms); }
 ...
 */
long random(long min, long max) { return mgos_rand_range((float)min, (float)max); }
