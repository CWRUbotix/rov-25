...
bool RHSPIDriver::init() {
  ...
  if (_slaveSelectPin != 0xff) pinMode(_slaveSelectPin, OUTPUT);
...
uint8_t RH_INTERRUPT_ATTR RHSPIDriver::spiBurstRead(uint8_t reg, uint8_t* dest, uint8_t len) {
  ...
  while (len--) *dest++ = _spi.transfer(0);
  deselectSlave();
  _spi.endTransaction();
  ATOMIC_BLOCK_END;
  return status;
...
void RHSPIDriver::setSlaveSelectPin(uint8_t slaveSelectPin) { _slaveSelectPin = slaveSelectPin; }

void RHSPIDriver::spiUsingInterrupt(uint8_t interruptNumber) {
  _spi.usingInterrupt(interruptNumber);
}

void RHSPIDriver::selectSlave() { digitalWrite(_slaveSelectPin, LOW); }

void RHSPIDriver::deselectSlave() { digitalWrite(_slaveSelectPin, HIGH); }
