...
bool RHEncryptedDriver::send(const uint8_t* data, uint8_t len) {
  if (len > maxMessageLength()) return false;
    ...
    #endif
  {
    ...
    if (k == 0)
      _cipheringBlocks.inputBlock[h++] =
        len;  // put in first byte of first block the message length
    #endif
    while (h < blockSize) {
    ...
    _blockcipher.encryptBlock(
      &_buffer[k * blockSize], _cipheringBlocks.inputBlock);  // Cipher that message into _buffer
  }
  ...
  for (int i = 0; i < nbMsg; i++) {
    ...
    for (k = 0; k < nbBpM && k * blockSize < len; k++) {
      ...
      _blockcipher.encryptBlock(
        &_buffer[k * blockSize], _cipheringBlocks.inputBlock);  // Cipher that message into buffer
    }
