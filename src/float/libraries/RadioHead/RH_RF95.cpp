...
void RH_RF95::handleInterrupt() {
  ...
  if (
    _mode == RHModeRx && ((irq_flags & (RH_RF95_RX_TIMEOUT | RH_RF95_PAYLOAD_CRC_ERROR)) ||
...
void RH_RF95::setSpreadingFactor(uint8_t sf) {
  ...
  spiWrite(
    ...
    (spiRead(RH_RF95_REG_1E_MODEM_CONFIG2) & ~RH_RF95_SPREADING_FACTOR) | sf);
...
void RH_RF95::setSignalBandwidth(long sbw) {
  ...
  spiWrite(
    RH_RF95_REG_1D_MODEM_CONFIG1, (spiRead(RH_RF95_REG_1D_MODEM_CONFIG1) & ~RH_RF95_BW) | bw);
  // check if low data rate bit should be set or cleared
  setLowDatarate();
}
...
void RH_RF95::setPayloadCRC(bool on) {
  ...
  uint8_t current =
    spiRead(RH_RF95_REG_1E_MODEM_CONFIG2) & ~RH_RF95_PAYLOAD_CRC_ON;  // mask off the CRC
