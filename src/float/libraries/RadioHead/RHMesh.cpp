...
////////////////////////////////////////////////////////////////////
// Constructors
RHMesh::RHMesh(RHGenericDriver& driver, uint8_t thisAddress) : RHRouter(driver, thisAddress) {}
...
uint8_t RHMesh::sendtoWait(uint8_t* buf, uint8_t len, uint8_t address, uint8_t flags) {
  if (len > RH_MESH_MAX_MESSAGE_LEN) return RH_ROUTER_ERROR_INVALID_LENGTH;

  if (address != RH_BROADCAST_ADDRESS) {
    RoutingTableEntry* route = getRouteTo(address);
    if (!route && !doArp(address)) return RH_ROUTER_ERROR_NO_ROUTE;
  }
...
bool RHMesh::doArp(uint8_t address) {
  ...
  uint8_t error =
    RHRouter::sendtoWait((uint8_t*)p, sizeof(RHMesh::MeshMessageHeader) + 2, RH_BROADCAST_ADDRESS);
  if (error != RH_ROUTER_ERROR_NONE) return false;
...
void RHMesh::peekAtMessage(RoutedMessage* message, uint8_t messageLen) {
  ...
  if (messageLen > 1 && m->msgType == RH_MESH_MESSAGE_TYPE_ROUTE_DISCOVERY_RESPONSE) {
    ...
    for (i = 0; i < numRoutes; i++)
      if (d->route[i] == _thisAddress) break;
    i++;
    while (i < numRoutes) addRouteTo(d->route[i++], headerFrom());
  }
...
uint8_t RHMesh::route(RoutedMessage* message, uint8_t messageLen) {
  ...
  if (ret == RH_ROUTER_ERROR_NO_ROUTE || ret == RH_ROUTER_ERROR_UNABLE_TO_DELIVER) {
    ...
    if (message->header.source != _thisAddress) {
      ...
      ret = RHRouter::sendtoWait(
        (uint8_t*)p, sizeof(RHMesh::MeshMessageHeader) + 1, message->header.source);
    }
  }
  return ret;
}
...
////////////////////////////////////////////////////////////////////
bool RHMesh::recvfromAck(
  uint8_t* buf, uint8_t* len, uint8_t* source, uint8_t* dest, uint8_t* id, uint8_t* flags,
  uint8_t* hops) {
  ...
  if (RHRouter::recvfromAck(_tmpMessage, &tmpMessageLen, &_source, &_dest, &_id, &_flags, &_hops)) {
    ...
    if (tmpMessageLen >= 1 && p->msgType == RH_MESH_MESSAGE_TYPE_APPLICATION) {
      ...
      _dest == RH_BROADCAST_ADDRESS && tmpMessageLen > 1 &&
      ...
      if (_source == _thisAddress) return false;

      uint8_t numRoutes = tmpMessageLen - sizeof(MeshMessageHeader) - 2;
      uint8_t i;
      // Are we already mentioned?
      for (i = 0; i < numRoutes; i++)
        if (d->route[i] == _thisAddress) return false;  // Already been through us. Discard

      addRouteTo(
        _source, headerFrom());  // The originator needs to be added regardless of node type
      ...
      if (_isa_router) {
        for (i = 0; i < numRoutes; i++) addRouteTo(d->route[i], headerFrom());
      }
...
////////////////////////////////////////////////////////////////////
bool RHMesh::recvfromAckTimeout(
  uint8_t* buf, uint8_t* len, uint16_t timeout, uint8_t* from, uint8_t* to, uint8_t* id,
  uint8_t* flags, uint8_t* hops) {
  unsigned long starttime = millis();
  int32_t timeLeft;
  while ((timeLeft = timeout - (millis() - starttime)) > 0) {
    if (waitAvailableTimeout(timeLeft)) {
      if (recvfromAck(buf, len, from, to, id, flags, hops)) return true;
      YIELD;
    }
  }
  return false;
