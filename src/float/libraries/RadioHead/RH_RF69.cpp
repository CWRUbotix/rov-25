...
#define CONFIG_NOWHITE                                                               \
  (RH_RF69_PACKETCONFIG1_PACKETFORMAT_VARIABLE | RH_RF69_PACKETCONFIG1_DCFREE_NONE | \
...
void RH_RF69::setIdleMode(uint8_t idleMode) { _idleMode = idleMode; }
...
bool RH_RF69::init() {
  ...
  if (_deviceType == 00 || _deviceType == 0xff) return false;
...
int8_t RH_RF69::temperatureRead() {
  ...
  while (spiRead(RH_RF69_REG_4E_TEMP1) &
         RH_RF69_TEMP1_TEMPMEASRUNNING);       // Wait for the measurement to complete
...
bool RH_RF69::send(const uint8_t* data, uint8_t len) {
  if (len > RH_RF69_MAX_MESSAGE_LEN) return false;
  ...
  digitalWrite(_slaveSelectPin, LOW);
  _spi.transfer(
    RH_RF69_REG_00_FIFO | RH_RF69_SPI_WRITE_MASK);  // Send the start address with the write mask on
...
bool RH_RF69::printRegisters() {
  uint8_t i;
  for (i = 0; i < 0x50; i++) printRegister(i);
