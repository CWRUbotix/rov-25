...
bool RH_Serial::available() {
  while (!_rxBufValid && _serial.available()) handleRx(_serial.read());
  return _rxBufValid;
}
...
void RH_Serial::waitAvailable(uint16_t polldelay) {
  ...
  while (!available()) _serial.waitAvailable();
  #else
...
bool RH_Serial::send(const uint8_t* data, uint8_t len) {
  if (len > RH_SERIAL_MAX_MESSAGE_LEN) return false;
...
uint8_t RH_Serial::maxMessageLength() { return RH_SERIAL_MAX_MESSAGE_LEN; }
