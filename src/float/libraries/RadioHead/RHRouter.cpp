...
bool RHRouter::init() {
  ...
  if (ret) _max_hops = RH_DEFAULT_MAX_HOPS;
  return ret;
}

////////////////////////////////////////////////////////////////////
void RHRouter::setMaxHops(uint8_t max_hops) { _max_hops = max_hops; }

////////////////////////////////////////////////////////////////////
void RHRouter::setIsaRouter(bool isa_router) { _isa_router = isa_router; }
////////////////////////////////////////////////////////////////////
    ...
    do {
      if (_routes[i].state == Valid) {
        ...
        *lastIndex_p = i;
        ...
        i++;
        if (i >= RH_ROUTING_TABLE_SIZE) stop = true;  // no more entries
      }
    }
    while (!stop);
  }
...
uint8_t RHRouter::sendtoFromSourceWait(
  uint8_t* buf, uint8_t len, uint8_t dest, uint8_t source, uint8_t flags) {
...
uint8_t RHRouter::route(RoutedMessage* message, uint8_t messageLen) {
  ...
  if (message->header.dest != RH_BROADCAST_ADDRESS) {
    ...
    if (!route) return RH_ROUTER_ERROR_NO_ROUTE;
...
bool RHRouter::recvfromAck(
  ...
  uint8_t _id;
  uint8_t _flags;
  if (RHReliableDatagram::recvfromAck(
        (uint8_t*)&_tmpMessage, &tmpMessageLen, &_from, &_to, &_id, &_flags)) {
    ...
    if (
      _tmpMessage.header.dest == _thisAddress || _tmpMessage.header.dest == RH_BROADCAST_ADDRESS) {
      // Deliver it here
      ...
      _tmpMessage.header.dest != RH_BROADCAST_ADDRESS && _tmpMessage.header.hops++ < _max_hops) {
      ...
      if (_isa_router) route(&_tmpMessage, tmpMessageLen);
    }
...
////////////////////////////////////////////////////////////////////
bool RHRouter::recvfromAckTimeout(
  uint8_t* buf, uint8_t* len, uint16_t timeout, uint8_t* source, uint8_t* dest, uint8_t* id,
  uint8_t* flags, uint8_t* hops) {
  unsigned long starttime = millis();
  int32_t timeLeft;
  while ((timeLeft = timeout - (millis() - starttime)) > 0) {
    if (waitAvailableTimeout(timeLeft)) {
      if (recvfromAck(buf, len, source, dest, id, flags, hops)) return true;
    }
    YIELD;
  }
  return false;
