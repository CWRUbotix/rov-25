...
bool RingBuffer::write(uint8_t ch) {
  ...
  _buffer[_head] = ch;
  if (++_head >= ARDUINO_RINGBUFFER_SIZE) _head = 0;
  return true;
}
...
uint8_t RingBuffer::read() {
  ...
  if (++_tail >= ARDUINO_RINGBUFFER_SIZE) _tail = 0;
  return ret;
}
...
int HardwareSerial::available(void) { return !_rxRingBuffer.isEmpty(); }

int HardwareSerial::read(void) { return _rxRingBuffer.read(); }
