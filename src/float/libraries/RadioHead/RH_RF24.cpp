...
void RH_RF24::setIdleMode(uint8_t idleMode) { _idleMode = idleMode; }
...
bool RH_RF24::init() {
  ...
  if (
    _deviceType != 0x4460 && _deviceType != 0x4461 && _deviceType != 0x4463 &&
    _deviceType != 0x4464)
...
bool RH_RF24::available() {
  ...
  if (!_rxBufValid) setModeRx();  // Make sure we are receiving
  return _rxBufValid;
}
...
bool RH_RF24::recv(uint8_t* buf, uint8_t* len) {
  ...
  if (buf && len && (_bufLen >= RH_RF24_HEADER_LEN)) {
    ATOMIC_BLOCK_START;
    if (*len > _bufLen - RH_RF24_HEADER_LEN) *len = _bufLen - RH_RF24_HEADER_LEN;
...
bool RH_RF24::send(const uint8_t* data, uint8_t len) {
  if (len > RH_RF24_MAX_MESSAGE_LEN) return false;
...
void RH_RF24::sendNextFragment() {
  if (_txBufSentIndex < _bufLen) {
    ...
    if (len > fifo_info[1]) len = fifo_info[1];
...
void RH_RF24::readNextFragment() {
  ...
  uint8_t l = fifo_len;
  while (l--) *p++ = _spi.transfer(0);
  digitalWrite(_slaveSelectPin, HIGH);
  _spi.endTransaction();
  _bufLen += fifo_len;
}

uint8_t RH_RF24::maxMessageLength() { return RH_RF24_MAX_MESSAGE_LEN; }

// Sets registers from a canned modem configuration structure
void RH_RF24::setModemRegisters(const ModemConfig* config) {
#ifdef RH_HAVE_SERIAL
  Serial.println(
    "Programming Error: setModemRegisters is obsolete. Generate custom radio config file with WDS "
    "instead");
#endif
...
void RH_RF24::setModeRx() {
  if (_mode != RHModeRx) {
    ...
    uint8_t rx_config[] = {
      0x00, RH_RF24_CONDITION_RX_START_IMMEDIATE, 0x00, 0x00, _idleMode, _idleMode, _idleMode};
...
void RH_RF24::setModeTx() {
  if (_mode != RHModeTx) {
    ...
    uint8_t tx_params[] = {
      0x00, (uint8_t)((_idleMode << 4) | RH_RF24_CONDITION_RETRANSMIT_NO |
...
void RH_RF24::setTxPower(uint8_t power) {
  ...
  if (_deviceType == 0x4460) {
    ...
    if (power > 0x4f) power = 0x4f;
  }
  else if (_deviceType == 0x4461) {
    // 0x7f = 16dBm
    pa_bias_clkduty = 0xc0;
    if (power > 0x7f) power = 0x7f;
  }
...
bool RH_RF24::command(
  uint8_t cmd, const uint8_t* write_buf, uint8_t write_len, uint8_t* read_buf, uint8_t read_len) {
  bool done = false;
  ...
  if (write_buf && write_len) {
    while (write_len--) _spi.transfer(*write_buf++);
  }
  ...
  for (count = 0; !done && count < RH_RF24_CTS_RETRIES; count++) {
    ...
    if (_spi.transfer(0) == RH_RF24_REPLY_CTS) {
      ...
      if (read_buf && read_len) {
        while (read_len--) *read_buf++ = _spi.transfer(0);
      }
      done = true;
    }
    // Sigh, the RFM26 at least has problems if we deselect too quickly :-(
...
bool RH_RF24::set_properties(uint16_t firstProperty, const uint8_t* values, uint8_t count) {
  ...
  uint8_t i;
  for (i = 0; i < 12 && i < count; i++) buf[3 + i] = values[i];  // DATAn
