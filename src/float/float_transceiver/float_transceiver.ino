...
const uint32_t ASCEND_TIME =
    0; // Disable ascend times now that we're properly ballasted
...
void loop() {
  ...
  if (stageIs(StageType::WaitDeploying) &&
    ...
    snprintf(judgePacketBuffer, JUDGE_PKT_SIZE, "ROS:SINGLE:%d:%lu,%d.%04d\0",
             TEAM_NUM, previousPressureReadTime, intComponent, fracComponent);
  ...
  if (!isSurfaced() &&
    ...
    previousPressureReadTime = millis();
    memcpy(packets[profileHalf] + packetIndex, &previousPressureReadTime,
           sizeof(uint32_t));
  ...
  bool suckingHitLimitSwitch =
      stageIs(StageType::Suck) && !digitalRead(LIMIT_FULL);
  bool pumpingHitLimitSwitch =
      stageIs(StageType::Pump) && !digitalRead(LIMIT_EMPTY);
  bool shouldSubmerge = isSurfaced() && submergeReceived;

  if (shouldSubmerge || stageTimedOut || suckingHitLimitSwitch ||
      pumpingHitLimitSwitch || isStartingStage) {
    serialPrintf("Ending stage #%d, type: %d, start time: %l, max wait time: "
                 "%l, current time: %l\n",
