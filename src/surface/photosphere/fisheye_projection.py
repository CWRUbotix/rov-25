...
for row_index, row in enumerate(projection):
    for col_index in enumberate(row):
        ...
        if col_index < left_seam[0] or col_index > right_seam[1]:
            ...
            coord[0] = (
                unit_to_normal_grid(coord[0], fisheye_images[1].diameter) + fisheye_images[1].top
            )
            coord[1] = (
                unit_to_normal_grid(coord[1], fisheye_images[1].diameter) + fisheye_images[1].left
            )
        ...
        # if it is in the inner image fill in the pixel from image 1
            ...
            coord[0] = (
                unit_to_normal_grid(coord[0], fisheye_images[0].diameter) + fisheye_images[0].top
            )
            coord[1] = (
                unit_to_normal_grid(coord[1], fisheye_images[0].diameter) + fisheye_images[0].left
            )
        ...
        # if it is in the overlapping area calculate the blur
            ...
            coord1[0] = (
                unit_to_normal_grid(coord1[0], fisheye_images[0].diameter) + fisheye_images[0].top
            )
            coord1[1] = (
                unit_to_normal_grid(coord1[1], fisheye_images[0].diameter) + fisheye_images[0].left
            )
            coord2[0] = (
                unit_to_normal_grid(coord2[0], fisheye_images[1].diameter) + fisheye_images[1].top
            )
            coord2[1] = (
                unit_to_normal_grid(coord2[1], fisheye_images[1].diameter) + fisheye_images[1].left
            )
            ...
            row[col_index] = fisheye_images[0].img[coord1[0]][coord1[1]] * alpha + fisheye_images[
                1
            ].img[coord2[0]][coord2[1]] * (1 - alpha)
